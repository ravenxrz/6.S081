...
procinit(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++) {
    ...
    // if (pa == 0)
    //   panic("kalloc");
    // // map kstack address to page table will be done in allocproc
    // uint64 va = KSTACK((int)(p - proc)); // 为kernel stack分配va，同时还附加了一个kernel
    //                                      // stack-guard标记，可以看kernel的mem layout
...
{
  uvmunmap(pagetable, TRAMPOLINE, 1, 0); // TODO：为什么这两个页面要保留，之后的trap课程应该会说
